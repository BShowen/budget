...
export function ExpenseLedgerSelection({
  ledger,
  selected,
  selectLedger,
  deselectLedger,
  ...
  const { leftToSpend } = useExpenseLedger({ ledgerId: ledger._id });
  return (
    <div
      onClick={
        selected
      ...
      className={`w-full flex flex-row justify-start items-center min-h-12 ps-5 lg:hover:cursor-pointer bg-white dark:bg-dark-mode-bg-1 py-2 gap-1 ${
      ...
      <div className="flex-col justify-start items-stretch">
        ...
        <div className="w-full flex flex-row justify-start items-center gap-1">
          <p className="text-xs font-medium text-color-light-gray">
            {toDollars(leftToSpend)} left to spend
          </p>
        </div>
      </div>
    </div>
